...
final class CacheStoreTests: XCTestCase {
    func testCachedModelsKeepStableEntityNamesAcrossDiskReopen() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("archermes-cache-\(UUID().uuidString)", isDirectory: true)
        ...
        do {
            ...
            context.insert(CachedMessage(
                serverURLString: "https://rename-fixture.invalid",
                sessionID: "rename-fixture",
                message: ChatMessage(
                    role: "assistant", content: "module-cache-roundtrip",
                    timestamp: 0, messageId: "rename-fixture-message"
                ),
                sortIndex: 0
            ))
            try context.save()
        ...
        let reopened = try ModelContainer(for: CachedSession.self, CachedMessage.self, configurations: configuration)
        let messages = try ModelContext(reopened).fetch(FetchDescriptor<CachedMessage>())
        XCTAssertEqual(messages.count, 1)
        XCTAssertEqual(messages.first?.content, "module-cache-roundtrip")
        XCTAssertEqual(messages.first?.sessionID, "rename-fixture")
        XCTAssertEqual(Schema.entityName(for: CachedSession.self), "CachedSession")
        XCTAssertEqual(Schema.entityName(for: CachedMessage.self), "CachedMessage")
    }
