...
final class LiveActivityTests: XCTestCase {
    ...
    func testBuildsAndParsesSessionDeepLink() throws {
        let url = try XCTUnwrap(ARCHermesDeepLink.sessionURL(sessionID: "session-abc"))
        let scheme = ARCHermesDeepLink.scheme

        XCTAssertEqual(url.scheme, scheme)
        XCTAssertEqual(url.host, "session")
        XCTAssertEqual(ARCHermesDeepLink.sessionID(from: url), "session-abc")
        XCTAssertEqual(ARCHermesDeepLink.sessionID(from: URL(string: "\(scheme)://session/session-xyz")!), "session-xyz")
        XCTAssertNil(ARCHermesDeepLink.sessionID(from: ARCHermesShareDraft.openURL))
    }

    func testSessionDeepLinkURLPercentEncodesSessionID() throws {
        let sessionID = "session & /?=✓"
        let url = try XCTUnwrap(ARCHermesDeepLink.sessionURL(sessionID: sessionID))
