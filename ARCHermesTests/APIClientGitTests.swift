...
final class APIClientGitTests: APIClientTestCase {
    ...
    func testCreateBranchSendsNewModeNotLocal() async throws {
        let client = makeClient { request in
            ...
            XCTAssertEqual(body["new_branch"] as? String, "archermes/test2")
            return apiTestJSONResponse(#"{"ok":true,"current_branch":"archermes/test2","status":{"is_git":true,"branch":"archermes/test2"},"branches":{"is_git":true,"current":"archermes/test2"}}"#, for: request)
        }

        let target = GitCheckoutTarget(ref: "main", mode: .local, newBranch: "archermes/test2")
        let response = try await client.gitCheckout(sessionID: "abc123", target: target)
        XCTAssertEqual(response.currentBranch, "archermes/test2")
    }
