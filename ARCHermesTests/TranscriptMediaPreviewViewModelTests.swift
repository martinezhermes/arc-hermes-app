...
final class TranscriptMediaPreviewViewModelTests: XCTestCase {
    ...
    func testSaveVideoFileToPhotoLibraryIntegration() async throws {
        guard ProcessInfo.processInfo.environment["ARCHERMES_RUN_PHOTOS_INTEGRATION"] == "1" else {
            throw XCTSkip("Set ARCHERMES_RUN_PHOTOS_INTEGRATION=1 to modify the simulator photo library.")
        }

        let suppliedPath = ProcessInfo.processInfo.environment["ARCHERMES_PHOTOS_TEST_VIDEO_PATH"]
        let fileURL: URL
        if let suppliedPath {
