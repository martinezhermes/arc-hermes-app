...
final class ShareInputReaderTests: XCTestCase {
    ...
    func testOversizedImageDataProducesNoAttachment() async {
        let oversized = Data(count: ARCHermesShareDraft.maximumSharedAttachmentBytes + 1)
    ...
    func testAttachmentsCappedAtSharedLimit() async {
        let providers = (0...ARCHermesShareDraft.maximumSharedAttachmentCount).map { index in
            legacyProvider(item: Data([UInt8(index)]) as NSData, typeIdentifier: UTType.png.identifier)
        }
        XCTAssertEqual(providers.count, ARCHermesShareDraft.maximumSharedAttachmentCount + 1)

        let input = await ShareInputReader.input(from: providers)

        XCTAssertEqual(input.attachments.count, ARCHermesShareDraft.maximumSharedAttachmentCount)
    }
