...
final class ShareViewController: UIViewController {
    ...
    private func configureView() {
        ...
        statusLabel.text = "Opening ARC Hermes..."
    ...
    private func saveDraftAndOpenHermes() async {
        ...
        let draft = ARCHermesShareDraft.draftText(textSnippets: input.textSnippets, urls: input.urls)

        guard !draft.isEmpty || !input.attachments.isEmpty else {
            showStatus("ARC Hermes accepts text, URLs, images, PDFs, and files up to 20 MB.")
            completeRequest(after: 0.8)
            return
        }

        guard let directory = ARCHermesShareDraft.containerURL() else {
            showStatus("Could not access ARC Hermes storage.")
            completeRequest(after: 0.8)
            return
        }

        do {
            try ARCHermesShareDraft.savePendingImport(draft: draft, attachments: input.attachments, in: directory)
        } catch {
    ...
    private func openMainApp() {
        let url = ARCHermesShareDraft.openURL
        ...
        extensionContext?.open(url, completionHandler: { [weak self] success in
            Task { @MainActor [weak self] in
                ...
                if success {
                    ...
                    self.openMainAppViaWorkaround(url)
                }
            }
        })
    }

    private func openMainAppViaWorkaround(_ url: URL) {
        ...
        showStatus("Shared content saved. Open ARC Hermes manually.")
