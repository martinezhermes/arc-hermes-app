...
final class AppIntentRouter {
    ...
    /// pass the optional `ARCHermesDeepLink` builders without unwrapping.
...
/// Shortcuts, Spotlight, and Siri via `ARCHermesShortcuts`.
struct NewChatIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult {
        AppIntentRouter.shared.requestDeepLink(ARCHermesDeepLink.newChatURL)
        return .result()
    }
}
...
struct NewChatVoiceIntent: AppIntent {
    ...
    @MainActor
    func perform() async throws -> some IntentResult {
        AppIntentRouter.shared.requestDeepLink(ARCHermesDeepLink.newChatVoiceURL)
        return .result()
    }
}
...
struct NewChatInProfileIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult {
        AppIntentRouter.shared.requestDeepLink(
            ARCHermesDeepLink.newChatInProfileURL(profileName: profile.id)
        )
        return .result()
    }
}
...
struct ARCHermesShortcuts: AppShortcutsProvider {
