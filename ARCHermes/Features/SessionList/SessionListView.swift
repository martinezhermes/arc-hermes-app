...
struct SessionListView: View {
    ...
    private var newSessionButton: some View {
        HapticButton(feedbackStyle: .medium) {
            ...
            .padding(.vertical, 10)
            .frame(minHeight: 48)
            // Keep the padding, rounded ends, and icon-to-text gap tappable.
    ...
    private var sceneActions: ARCHermesSceneActions {
    ...
    private func openPendingSharedImportIfNeeded() {
        ...
        let draft = ARCHermesShareDraft.composerDraft(from: sharedImport.draft)
...
#Preview("Hermes Header Logo") {
    VStack(spacing: 16) {
        ForEach(HeaderLogoColor.presets.prefix(4)) { preset in
            ARCHermesHeaderLogo(selectedColor: preset.color)
                .frame(width: 220)
        }
    }
    .padding(24)
